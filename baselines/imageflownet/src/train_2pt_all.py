...
def train(config: AttributeHashmap):
    ...
    # Data augmentation DISABLED (per request): the FAF is fed as cropped (768->620) + resized
    # (target_dim) + deterministic intensity-normalised ONLY -- no HorizontalFlip /
    # ShiftScaleRotate / RandomBrightnessContrast / additive noise. Restore the A.Compose below
    # to re-enable. NOTE: the aug_transform branch is only used by the SimSiam invariance loss
    # (coeff_invariance>0), which is OFF for these runs.
    # train_transform = A.Compose(
    #     [A.HorizontalFlip(p=0.5),
    #      A.ShiftScaleRotate(shift_limit=0.1, scale_limit=0.1, rotate_limit=30, p=0.5),
    #      A.RandomBrightnessContrast(brightness_limit=0.1, contrast_limit=0.1, p=0.5)],
    #     additional_targets={'image_other': 'image'})
    ...
    # ImageFlowNet's training-duration criterion (official train_predictor.py): stop early when the
    # validation prediction PSNR stops improving for `patience` epochs (mode='max'), capped at
    # max_epochs. The best-by-val-PSNR checkpoint is the selected one.
    early_stopper = EarlyStopping(mode='max', patience=config.patience, percentage=False)
    ...
    # TensorBoard (consistent with the other GAP-INR baselines). `tensorboard --logdir <save_folder>/tb`.
    try:
        from torch.utils.tensorboard import SummaryWriter
        tb_writer = SummaryWriter(log_dir=config.save_folder + 'tb/')
    except Exception:
        tb_writer = None
    ...
    for epoch_idx in tqdm(range(config.max_epochs)):
        ...
        # Validate / checkpoint / early-stop only every config.val_every epochs (the diffusion
        # sampling in I2SB validation is the bottleneck); always validate on the final epoch so the
        # last checkpoint + early-stop check still run. NB: early-stop patience is then counted in
        # VALIDATIONS, so effective patience = config.patience * config.val_every epochs.
        if (epoch_idx + 1) % config.val_every != 0 and (epoch_idx + 1) != config.max_epochs:
            continue
        ...
        with ema.average_parameters():
            ...
            if config.model == 'I2SBUNet':
                val_recon_psnr, val_pred_psnr, val_seg_dice_xT = \
                    val_epoch_I2SB(config=config, device=device, val_set=val_set, model=model, epoch_idx=epoch_idx, max_t=max_t, tb_writer=tb_writer)
            else:
                val_recon_psnr, val_pred_psnr, val_seg_dice_xT = \
                    val_epoch(config=config, device=device, val_set=val_set, model=model, epoch_idx=epoch_idx, tb_writer=tb_writer)

        if tb_writer is not None:
            tb_writer.add_scalar('val/recon_psnr', val_recon_psnr, epoch_idx)
            tb_writer.add_scalar('val/pred_psnr', val_pred_psnr, epoch_idx)
            tb_writer.add_scalar('val/seg_dice_xT', val_seg_dice_xT, epoch_idx)
        ...
        # Early stopping on validation prediction PSNR (ImageFlowNet's criterion). Capped by max_epochs.
        if early_stopper.step(val_pred_psnr):
            log('Early stopping criterion met (val pred PSNR no improvement for %d epochs). '
                'Ending training at epoch %d.' % (config.patience, epoch_idx + 1),
...
def train_epoch(config: AttributeHashmap,
    ...
    for iter_idx, (images, timestamps) in enumerate(tqdm(train_set)):
        ...
        x_start_noisy, x_end_noisy = x_start, x_end  # add_random_noise DISABLED (per request)
    ...
    for iter_idx, (images, timestamps) in enumerate(tqdm(val_set)):
        ...
        if shall_plot:
            save_path_fig_sbs = '%s/val/figure_log_epoch%s_sample%s.png' % (
                config.save_folder, str(epoch_idx + 1).zfill(5), str(iter_idx + 1).zfill(5))
            fig = plot_side_by_side(t_list, x0_true, xT_true, x0_recon, xT_recon, xT_pred, save_path_fig_sbs,
                                    x0_true_seg=x0_seg, xT_pred_seg=xT_pred_seg, xT_true_seg=xT_seg,
                                    return_fig=tb_writer is not None)
            if tb_writer is not None:
                # Same panel that is written to disk, mirrored into TensorBoard. add_figure closes it.
                tb_writer.add_figure('val/panel_sample%s' % str(iter_idx + 1).zfill(5), fig, epoch_idx)
                # Standardized cross-method monitor panel (same layout/colormaps as GAP-INR/gliomagrowth/
                # NISF + the Phase-4 figure). NB: IFN's "GT mask" is the segmentor's output on the GT
                # image (no real GA mask is loaded at train time) -> labelled as such.
                try:
                    ...
                    monitor_panel.log_panel(
                        tb_writer, 'val/monitor/sample%s' % str(iter_idx + 1).zfill(5), epoch_idx,
                        xT_true, xT_pred, xT_seg, xT_pred_seg,
                        title='ImageFlowNet sample%s' % str(iter_idx + 1).zfill(5),
                        mask_note='segmentor GT (no real mask in val)')
                except Exception as _e:
                    print('[monitor_panel] IFN val skip:', _e)
...
def plot_side_by_side(t_list, x0_true, xT_true, x0_recon, xT_recon, xT_pred, save_path: str,
                      x0_true_seg=None, xT_pred_seg=None, xT_true_seg=None, return_fig: bool = False):
    ...
    # Rendered EXACTLY like the GAP-INR error maps: single-channel magma, fixed vmin=0/vmax=1 (images
    # are already in [0,1]), so the colour scale is identical and comparable across the two figures.
    diff_gt = np.abs(x0_true - xT_true).mean(axis=-1) if x0_true.ndim == 3 else np.abs(x0_true - xT_true)
    diff_pred = np.abs(xT_pred - xT_true).mean(axis=-1) if xT_pred.ndim == 3 else np.abs(xT_pred - xT_true)
    ax = fig_sbs.add_subplot(2, 6, 4)
    ax.imshow(diff_gt, cmap='magma', vmin=0, vmax=1)
    ...
    ax.imshow(diff_pred, cmap='magma', vmin=0, vmax=1)
    ...
    if return_fig:
        # Caller (e.g. TensorBoard add_figure, which closes by default) owns the figure now.
        return fig_sbs
...
if __name__ == '__main__':
    ...
    parser.add_argument('--crop-size', default=620, type=int,
                        help="centered crop of the native 768 BEFORE resize to target-dim. 620 = the "
                             "registration-frame crop (default, shared with GAP-INR faf_ga_620). Set 768 "
                             "for NO crop (pure resize 768->target, e.g. the 256 ImageFlowNet-style track).")
    ...
    parser.add_argument('--val-every', default=1, type=int,
                        help='run validation/checkpoint/early-stop every N epochs (default 1 = every '
                             'epoch). Set >1 to skip the expensive diffusion sampling on off-epochs; '
                             'early-stop patience is then counted in validations.')
    parser.add_argument('--patience', default=8, type=int,
                        help='early-stop patience on val pred PSNR (ImageFlowNet default 8).')
