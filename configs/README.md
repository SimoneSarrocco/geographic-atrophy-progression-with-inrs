...
| File | Role |
|---|---|
| `config_atlas.yaml` | **Model / optimizer / training** config: the INR decoder (architecture, `latent_dim`, `hidden_size`, SIREN `omega_*` schedule, `out_dim = [#intensity, #seg]`, seg head), the optimizer (`lr_inr`, `lr_latent`, loss weights, test-time-adaptation early-stopping), `epochs`, `validate_every`, `n_samples`, and `atlas_gen`. It carries a top-level `config_data:` key naming which dataset section to use. |
...
`config_data:` field inside the chosen `--config_atlas` file. Any nested field can be overridden from
